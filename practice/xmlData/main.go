...
type CharacterNumber struct {
	...
	Positives    []string `xml:"positive"`
	Negatives    []string `xml:"negative"`
}
...
func (self *CharacterNumber) TrimSpace() {
	...
	for i := 0; i < len(self.Positives); i++ {
		self.Positives[i] = strings.TrimSpace(self.Positives[i])
	}
	for i := 0; i < len(self.Negatives); i++ {
		self.Negatives[i] = strings.TrimSpace(self.Negatives[i])
	}
}
...
func (self *CharacterNumber) Print() {
	...
	fmt.Println("Positives: " + strings.Join(self.Positives, ", "))
	fmt.Println("Negatives: " + strings.Join(self.Negatives, ", "))
	fmt.Println("-----------------------------------------------------------")
}
